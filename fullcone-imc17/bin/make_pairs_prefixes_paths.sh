...
BGPDUMP=./bin/bgpdump
SCRIPT=./bin/gzipped_prefixes_pairs_from_stdin.py
BGPLOG=/tmp/log/bgpdumps-$(date +%s)-${$}.log
...
if [ ! -f "pairs/$OUT.gz" ] || [ $(find "${SOURCEDIR}/${ORG}/${RRC}" -cnewer "pairs/$OUT.gz" -type f -name "*${DATE}.*" | wc -l) != 0 ] || [ ! -f "prefixes/${OUT}.gz" ] || [ ! -f "paths/${OUT}.gz" ]
then
